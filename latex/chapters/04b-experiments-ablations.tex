...
\begin{table}[h]
  ...
  (D) G-DINO & \textbf{0.3814} & 0.90 & 2h\,37m \\
  \midrule
...
\begin{tabular}{@{}lc@{}}
  ...
  \midrule
  Raw frame + mask overlay          & 0.412  \\
  Raw + overlay + crop                           & 0.411   \\
  Only tight crop                     & 0.180  \\
  \bottomrule
\end{tabular}
\end{table}
...
The waterfall placed naming at the head of the dead mass, and the first direction of \cref{subsec:gap-summary} asks for the scene context that a bare object name discards. We add a stage after Stage~1 that classifies the take into its scenario, maps that scenario to a curated list of plausible object names, and hands the list to Stage~2, so the generated description is anchored to a known vocabulary rather than invented from the overlay alone. The intervention is encouraging on the smaller subset, lifting IoU from 0.376 to 0.408 in \textit{Ego2Exo} and from 0.322 to 0.480 in \textit{Exo2Ego} on 10\% of the validation set, but the gain narrows to $+0.011$ and reverses to $-0.008$ on the 25\% subset (\cref{tab:abl-vlm-align}). 
...
\begin{table}
  ...
  \midrule
  \textit{Ego2Exo} & top-k confidence & Low $[0,0.25)$   &  0.015 & 0.064 & $+0.049$ \\
  \textit{Ego2Exo} & top-k confidence & Mid $[0.25,0.6)$ &  0.435 & 0.499 & $+0.063$ \\
  \textit{Ego2Exo} & top-k confidence & High $[0.6,1]$   & 0.870 & 0.676 & $-0.194$ \\
  \cmidrule(l){2-6}
  \textit{Ego2Exo} & bbox area & Low $[0,0.25)$   & 0.011 & 0.088 & $+0.077$ \\
  \textit{Ego2Exo} & bbox area & Mid $[0.25,0.6)$ &   0.431 & 0.318 & $-0.113$ \\
  \textit{Ego2Exo} & bbox area & High $[0.6,1]$   &  0.866 & 0.651 & $-0.215$ \\
  \midrule
  \textit{Exo2Ego} & top-k confidence & Low $[0,0.25)$   & 0.009 & 0.065 & $+0.056$ \\
  \textit{Exo2Ego} & top-k confidence & Mid $[0.25,0.6)$ &   0.435 & 0.494 & $+0.059$ \\
  \textit{Exo2Ego} & top-k confidence & High $[0.6,1]$   & 0.896 & 0.820 & $-0.076$ \\
  \cmidrule(l){2-6}
  \textit{Exo2Ego} & bbox area & Low $[0,0.25)$   &  0.007 & 0.051 & $+0.043$ \\
  \textit{Exo2Ego} & bbox area & Mid $[0.25,0.6)$ &   0.430 & 0.228 & $-0.202$ \\
  \textit{Exo2Ego} & bbox area & High $[0.6,1]$   &   0.902 & 0.685 & $-0.217$ \\
  \bottomrule
  \end{tabular}
\end{table}
