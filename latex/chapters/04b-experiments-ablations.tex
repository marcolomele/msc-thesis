...
Reading the waterfall back through \cref{chap:theory} clarifies why naming dominates. The language bridge of \cref{subsec:Stage2} asks a general VLM to name a target it sees only as a translucent overlay on a full frame. When that target occupies a fraction of a percent of the image, is ambiguous, or can have multiple names, the model describe the salient object based on the features it is able to extract. The bridge is only as good as the object description it carries over to the other view.
...
\begin{table}
\centering
\caption{What the VLM nominates on the dead cases ($\mathrm{seed\ IoU}=0$), as a proportion of all such cases per direction.}
