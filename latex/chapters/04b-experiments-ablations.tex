...
We analyse the pipeline along three dimensions, namely its architecture, the selection and number of anchors, and the quality of the descriptions Stage~2 produces. Each ablation isolates a single design decision or a single foundation model, locating where the pipeline gains accuracy and where it remains limited on a task none of its constituent models was built to solve. Each table below fixes its own experimental configuration and validation subset, so the numbers it reports are comparable within the table rather than across tables.
...
\begin{table}[h]
  ...
  (B) SAM3 Agent Loop & 35.5 & 21.30 & 59h\,15m \\
...
\begin{table}
    \centering
    \setlength{\tabcolsep}{4pt}
    \caption{Description quality judged by Gemma~4. A successful description gets both object name and properties correct.}
    \label{tab:abl-vlm}
    \begin{tabular}{l ccc}
      \toprule
...
\begin{table}[t]
  ...
  \begin{tabular}{llcc}
  \toprule
  Pipeline Stage & First failure & \textit{Ego2Exo} & \textit{Exo2Ego} \\
  \midrule
  2 & VLM names a different object        & 59\% & 81\% \\
  3 & Anchor frame lands on a tiny object   & 15\% & 11\% \\
  3 & Grounding DINO box misses the object & 10\% & 2\%  \\
  3 & Agent segments wrong despite a good box & 16\% & 5\% \\
  4 & Propagation                          & 0\%  & 0\%  \\
  \midrule
          & Total                                & 100\% & 100\% \\
  \bottomrule
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{latex/imgs/ch4/vlm-description-failure.png}
    \caption{Examples of VLM description failures. The first is a case where the target object is seen partially in the source frame. The second is a case where the description is not specific enough and locks on another object in the anchor frame.}
    \label{fig:vlm-failure}
\end{figure}

Therefore, the bulk of the loss is produced before SAM~3 ever runs, pointing to the need for more thorough analysis of VLM descriptions than we provide in \cref{sec:ablations}.

Reading the waterfall back through \cref{chap:theory} clarifies why naming dominates. The language bridge of \cref{subsec:Stage2} asks a general VLM to name a target it sees only as a translucent overlay on a full frame. When that target occupies a fraction of a percent of the image, is ambiguous, or can have multiple names, the model describes the salient object using the features it can extract. The bridge is only as good as the object description it carries over to the other view. \cref{fig:vlm-failure} presents two qualitative examples of this kind of failure. 
...
\begin{table}[t]
\centering
\caption{Effect of the descriptor-VLM input image format on segmentation
  quality (10\% of validation set, egoexo, displaying anchor-frame IoU). Replacing the full frame with a crop collapses IoU by mis-naming the isolated patch; a crop added
  alongside the full frame has no impact.}
\label{tab:vlm-input-format}
\begin{tabular}{@{}lc@{}}
  \toprule
  What the descriptor-VLM sees & IoU (egoexo) \\
  \midrule
...
\paragraph{Extending to more failures.}
...
The waterfall placed naming at the head of the dead mass, and the first direction of \cref{subsec:gap-summary} asks for the scene context that a bare object name discards. We add a stage after Stage~1 that classifies the take into its scenario, maps that scenario to a curated list of plausible object names, and hands the list to Stage~2, so the generated description is anchored to a known vocabulary rather than invented from the overlay alone. The intervention is encouraging on the smaller subset, lifting IoU from 37.6 to 40.8 in \textit{Ego2Exo} and from 32.2 to 48.0 in \textit{Exo2Ego} on 10\% of the validation set, but the gain narrows to $+1.1$ and reverses to $-0.8$ on the 25\% subset (\cref{tab:abl-vlm-align}). 
...
A second set of dead cases is linked with Grounding DINO and the agent locking on a large neighbour of the target. Cropping the source frame for the VLM already proved counterproductive in \cref{subsec:crop}, but that crop starved the description of context. Here we crop on the destination side instead, passing the Grounding DINO box to the SAM~3 agent as a hint and segmenting within it. In aggregate the hint hurts in all four combinations of direction and ranking, by $-4.4$ and $-4.3$ in \textit{Ego2Exo} and $-0.5$ and $-4.3$ in \textit{Exo2Ego}. Stratifying by the baseline IoU of each frame explains the loss (\cref{tab:abl-crop-strat}), since the hint rescues frames the pipeline was already failing, adding 4.3 to 7.7 in the low baseline band, yet corrupts frames the pipeline already handled, removing 7.6 to 21.7 in the high band, and the population the pipeline already handles is large enough to drive the mean negative. The thin payoff in the failing band is not a fault of the crop but of what precedes it. When the VLM mislabels the object, Grounding DINO confidently boxes the wrong one and the crop faithfully zooms into that wrong object, so the hint cannot recover a case that naming has already lost (\cref{subsec:Stage2-deepdive}). An unconditional hint is therefore the wrong tool, and only a gated variant that fires on the failing population without touching the rest could net positive.

\begin{table}
  \centering\small
  \setlength{\tabcolsep}{5pt}
  \caption{Destination crop hint stratified by the baseline IoU of each frame, 25\% validation. Rank is the anchor ranking rule, by G-DINO confidence (top-k confidence) or bounding box area (bbox area). The hint helps the low baseline band and harms the high band, and the large high band drives the negative mean.}
  \label{tab:abl-crop-strat}
  \begin{tabular}{lllrcc}
  \toprule
  Direction & Rank & Baseline band & IoU (Off) & IoU (On) & $\Delta$ \\
  \midrule
  \textit{Ego2Exo} & top-k confidence & Low $[0,0.25)$   &  1.5 & 6.4 & $+4.9$ \\
  \textit{Ego2Exo} & top-k confidence & Mid $[0.25,0.6)$ &  43.5 & 49.9 & $+6.3$ \\
  \textit{Ego2Exo} & top-k confidence & High $[0.6,1]$   & 87.0 & 67.6 & $-19.4$ \\
  \cmidrule(l){2-6}
  ...
  \midrule
  \textit{Exo2Ego} & top-k confidence & Low $[0,0.25)$   & 0.9 & 6.5 & $+5.6$ \\
  \textit{Exo2Ego} & top-k confidence & Mid $[0.25,0.6)$ &   43.5 & 49.4 & $+5.9$ \\
  \textit{Exo2Ego} & top-k confidence & High $[0.6,1]$   & 89.6 & 82.0 & $-7.6$ \\
  \cmidrule(l){2-6}
...
Egocentric frames are often dark, which suggested that part of the dead mass might be a problem of raw visibility. Raising the brightness and contrast of the egocentric frames before description and segmentation does not help, and instead lowers IoU by 3.7 in \textit{Ego2Exo} and 2.3 in \textit{Exo2Ego}, within the range of variation between runs (\cref{tab:abl-bright}). Visibility at the pixel level is therefore not the binding constraint, which is consistent with the earlier finding that the dead cases show the object plainly and fail at naming rather than at the perception of light (\cref{subsec:born-at-anchor}).
...
\vspace{4mm}
The above interventions localize constraints without solving them. Nonetheless, they provide valuable findings that deepen our understanding of the language as a bridge paradigm in the context of cross-view object correspondence. What's left is to collect these learnings together and identify directions for future research, which we do in \cref{chap:conclusions}.