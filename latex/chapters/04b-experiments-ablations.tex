...
\caption{Effect of the descriptor-VLM input image format on segmentation
  quality (10\% eof validation set, egoexo, displaying anchor-frame IoU). Replacing the full frame with a crop collapses IoU by mis-naming the isolated patch; a crop added
...
The waterfall placed naming at the head of the dead mass, and the first direction of \cref{subsec:gap-summary} asks for the scene context that a bare object name discards. We add a stage after Block~1 that classifies the take into its scenario, maps that scenario to a curated list of plausible object names, and hands the list to Block~2, so the generated description is anchored to a known vocabulary rather than invented from the overlay alone. The intervention is encouraging on the smaller subset, lifting IoU from 0.376 to 0.408 in \textit{Ego2Exo} and from 0.322 to 0.480 in \textit{Exo2Ego} on 10\% of the validation set, but the gain narrows to $+0.011$ and reverses to $-0.008$ on the 25\% subset (\cref{tab:abl-vlm-align}). 
