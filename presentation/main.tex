...
\begin{frame}{Task formulation}
  \textbf{Input:} a masklet in the source view $S$ — a binary mask for each annotated frame:
  \[
    \bigl\{ M^\tau_S \bigr\}_{\tau \in \mathcal{T}}, \qquad M^\tau_S \in \{0,1\}^{H \times W}
  \]
  \textbf{Output:} a mask for \emph{every} destination frame:
  \[
    \hat{M}^\tau_D \quad \forall\, \tau \in \{1,\ldots,T\}
  \]
  \medskip
  Two orthogonal sub-problems:
  \begin{itemize}
    \item \textbf{Cross-view transfer} — bridge the viewpoint gap. \emph{Hard.}
    \item \textbf{Temporal completeness} — cover every frame. \emph{Solved by trackers.}
  \end{itemize}
  \medskip
  Strategy: language bridge for transfer; SAM 2 for completeness.
\end{frame}
...
\begin{frame}{Related works}
  \begin{center}
    \includegraphics[width=0.82\textwidth]{latex/imgs/ch2/related-works.png}
  \end{center}
\end{frame}
\note{}
...
\begin{frame}{Stage 1: Source Frame Selection}
  \begin{itemize}
    \item Score each annotated frame by a visibility heuristic and take the highest.
  \end{itemize}
  \begin{center}
...
\begin{frame}{Stage 3.2: Agentic Segmentation}
  \begin{itemize}
    \item Iteratively, simplify the text prompt, segment candidates, and select the final mask against the original description.
  \end{itemize}
  \begin{center}
