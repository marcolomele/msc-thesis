...
\begin{frame}{Stage 1: source frame selection}
  \begin{itemize}
    \item Score each annotated frame by a visibility heuristic; take the highest.
  \end{itemize}
  \[
    s_t = 0.99\,a_t + 0.01\,c_t, \qquad
    a_t = \frac{A_t}{H \cdot W}
  \]
  \begin{itemize}
    \item $a_t$: normalised mask area (dominant term); $c_t$: mask centrality (tie-breaker).
    \item A larger object gives the VLM more signal to describe.
  \end{itemize}
\end{frame}
...
\begin{frame}{Stage 3.2: Agentic Segmentation}
  ...
  \end{itemize}
  \begin{center}
    \includegraphics[width=0.8\textwidth]{latex/imgs/ch3/block-3-2-focus.png}
  \end{center}
\end{frame}
