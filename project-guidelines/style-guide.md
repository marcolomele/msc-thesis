...
**Inter-paragraph logic.** Paragraphs chain by tension, not by sequence. The connector between paragraphs names the complication the previous paragraph left open and launches the response: "However," "To address this," "To this end," "In contrast," "Nevertheless." Pure additive chaining — "Moreover" paragraph after "Moreover" paragraph — signals a list rather than an argument and should be avoided. This applies with particular force in method-survey passages inside Theory sections, where each new family of models must be introduced as the answer to a failure the previous family could not resolve — not merely as the next item on a chronological list. The reader should feel the argument move forward under pressure, not simply accumulate.
...
**When surveying prior approaches within a theory subsection, each method is described then immediately limited — in the same paragraph, not a later one.** The pattern mirrors Related Work: [method name + author + year] → [what it does] → [what it cannot do or what cost it pays]. The limitation is the reason the next paradigm exists; withholding it until the next paragraph breaks the causal chain and makes the section read as a list. The transition between paradigms must name the open problem the prior family left: "The fundamental limitation was X; the memory-based paradigm addressed this by Y." The reader must feel why the story moves forward, not merely that it does.
